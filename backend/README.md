 ...
 **`Step 4`**. In the /backend folder, create a new file and rename as '.env' to store the BC Government Single Sign-on(SSO) secret. Copy '.env.template' contents to '.env'. All explanation/uses for env variables is given in comments in .env.template file
